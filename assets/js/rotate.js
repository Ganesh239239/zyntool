// Rotate IMAGE - Unique Logic
document.addEventListener('DOMContentLoaded', function() {
    console.log('✅ rotate.js loaded - ROTATE MODE');
    ...
    function processImage(quality) {
        // ROTATE: Shows degrees
        const degrees = Math.round(quality * 3.6);
        processedSize = originalSize;

        document.getElementById('progress-indicator').textContent = `${degrees}°`;
        document.getElementById('original-size').textContent = 'Original';
        document.getElementById('compressed-size').textContent = 
            `Rotated ${degrees}°`;

        updatePieChart(Math.round(degrees / 360 * 100), 'rotate');
    }
