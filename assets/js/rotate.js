// Rotate IMAGE - ROTATE MODE
document.addEventListener('DOMContentLoaded', function() {
    console.log('✅ rotate.js - ROTATE MODE');
    ...
    function processImage(quality) {
        // ROTATE
        const degrees = Math.round(quality * 3.6);
        processedSize = originalSize;

        document.getElementById('progress-number').textContent = `${degrees}°`;
        document.getElementById('progress-label').textContent = 'Rotation';
        document.getElementById('original-stat').textContent = '0°';
        document.getElementById('processed-stat').textContent = `${degrees}°`;
        document.getElementById('pie-percentage').textContent = `${degrees}°`;

        updatePieChart(Math.round(degrees / 360 * 100));
    }
    ...
    if (downloadBtn) {
        downloadBtn.addEventListener('click', function() {
            if (uploadedFile && uploadedFile.dataUrl) {
                ...
                link.download = 'rotate_' + uploadedFile.name;
                link.click();
            }
        });
    }
