// Watermark IMAGE - WATERMARK MODE
document.addEventListener('DOMContentLoaded', function() {
    console.log('✅ watermark.js - WATERMARK MODE');
    ...
    function processImage(quality) {
        // WATERMARK
        const opacity = quality;
        processedSize = Math.round(originalSize * 1.02);

        document.getElementById('progress-number').textContent = `${opacity}%`;
        document.getElementById('progress-label').textContent = 'Opacity';
        document.getElementById('original-stat').textContent = 'No watermark';
        document.getElementById('processed-stat').textContent = 'With watermark';
        document.getElementById('pie-percentage').textContent = `${opacity}%`;

        updatePieChart(opacity);
    }
    ...
    if (downloadBtn) {
        downloadBtn.addEventListener('click', function() {
            if (uploadedFile && uploadedFile.dataUrl) {
                ...
                link.download = 'watermark_' + uploadedFile.name;
                link.click();
            }
        });
    }
