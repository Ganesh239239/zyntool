// Watermark IMAGE - Unique Logic
document.addEventListener('DOMContentLoaded', function() {
    console.log('✅ watermark.js loaded - WATERMARK MODE');
    ...
    function processImage(quality) {
        // WATERMARK: Shows opacity
        const opacity = quality;
        processedSize = Math.round(originalSize * 1.02);

        document.getElementById('progress-indicator').textContent = `${opacity}%`;
        document.getElementById('original-size').textContent = 'No watermark';
        document.getElementById('compressed-size').textContent = 
            `With watermark`;

        updatePieChart(opacity, 'watermark');
    }
